...
- **创建型模式**：
   ...
   - **工厂方法模式**：提供一个创建对象的方法，由子类决定具体实现，例如不同的工厂生产不同的商品。
...
- **结构型模式**：
   ...
   - **桥接模式**：将抽象部分与实现部分分离，使它们可以独立变化。像桥一样连接了抽象和实现两个部分，使它们能独立变化而不影响对方。
...
- **行为型模式**：
   ...
   - **观察者模式**：允许对象监视另一对象的状态，类似于日常生活中的观察。当一个对象状态变化时，其所有依赖都会收到通知。
   - **状态模式**：当你的对象的行为依赖于其状态，而你不想用大量的if-else语句时，可以使用状态模式。它允许你在对象的状态改变时，改变其行为。例如，一个电灯有开和关两种状态，行为（亮度）会随状态改变。
